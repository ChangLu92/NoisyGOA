function [] = TestranNR2_hr2(dataseth,datasetr,goObj,GOs)
%NR 蛋白质噪声标记剔除
%   dataset是蛋白质功能数据库
%   goObj是GO数据库
%   ccGOs1是GO数据库中cc分支
fprintf('start %s  at %s\n,==Method:%s==',dataseth, datestr(now),'TestranNR2_hr');
load(dataseth);
load(datasetr);
...
t0=clock;
 [newGF, reRatio,preRatio]= randomNR2(gnd3, Ndata,childGOs, selGOs, num_perprotein_noise,noiseidx);  %随机寻找三个叶子节点
...
if rootGO==5575
evalstr=['save results',filesep,dataseth, '_ranNR_cc.mat precision stds prec_seq'];
end
if rootGO==3674
evalstr=['save results',filesep,dataseth, '_ranNR_mf.mat precision stds prec_seq'];
end
if rootGO==8150
evalstr=['save results',filesep,dataseth, '_ranNR_bp.mat precision stds prec_seq'];
end
eval(evalstr);
